...
def monthly_report(conn, year: int, month: int) -> Path:
    ...
    ws.append(["Date", "Pump House", "Name", "Category", "Task", "Note",
               "Photo", "Worker"])
    for it in items:
        if it["result"] != "issue":
            continue
        ...
        ws.append([insp["submitted_at"][:10], insp["pump_house"],
                   ...
                   it["note"] or "", "YES" if it["photo_file_id"] else "",
                   insp["worker_name"]])
    _style_header(ws, 8)
    _autosize(ws, {1: 12, 2: 11, 3: 26, 4: 26, 5: 45, 6: 40, 7: 7, 8: 22})
    ...
    for it in items:
        ...
        ws.append([insp["submitted_at"][:10], insp["pump_house"], cat["name"],
                   ...
                   RESULT_LABEL.get(it["result"], ""), it["note"] or "",
                   insp["worker_name"]])
        if it["result"] == "issue":
            for col in range(1, 10):
                ws.cell(row=ws.max_row, column=col).fill = ISSUE_FILL
    _style_header(ws, 9)
    _autosize(ws, {1: 12, 2: 11, 3: 26, 4: 4, 5: 50, 6: 14, 7: 8, 8: 35, 9: 22})
